...
int main() {   
 ...
 cout<<"\t\t Individual project is done by:\n";
cout<<"\tName\t\t          ID\n";
cout<<"\tAddisu Berihun\t\t1200800\n\n";
...
void update(string lp) {
		 char choice[10];
  car * temp = head;
    car *temp2=head;
  while (temp != NULL) {

    if (temp -> license_plate == lp)
      break;
    temp = temp -> next;
  ...
  if (head == NULL)
    ...
    for (temp = head; temp -> next != NULL; temp = temp -> next) {
      ...
      for (temp2 = temp -> next; temp2 != NULL; temp2 = temp2 -> next) {
        ...
        if (temp -> license_plate > temp2 -> license_plate || temp -> start > temp2 -> start || temp -> end > temp2 -> end) {
